#!/sbin/sh
Keys="hdmimode voutmode display_autodetect backlight_pwm zoom_rate colorattribute osd_reverse video_reverse suspend_hdmiphy cvbsmode disablehpd disable_vu7 touch_invert_x touch_invert_y test_mt_vid test_mt_pid max_freq_a53 governor_a53 enable_wol heartbeat sg_tablesize adjustScreenWay screenAlignment autoFramerate gpuScaleMode overlays overlay_resize"
	...
	do
		if [[ "$Keys" == *"$KEY"* ]]; then
			if [[ "Keys" == "overlays" ]]; then
				command echo "s/^$KEY=\".*\"/$KEY=\"pwm_ab pwm_ef ${VALUE:1}/" >> $SEDCOMMAND
			else
